...
def runAlgorithm(
    startBoard: board.Board, algorithm: int
) -> Tuple[List[board.Board], float]:
    """
...
def display_results(
    game: rushhour.RushHour, path: List[board.Board], run_time: float, algorithm: int
) -> None:
    ...
    print(
        f"The board was solved with {algorithm_name} in {len(moves)} steps and {run_time} seconds."
    )
